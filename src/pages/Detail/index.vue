          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="attr in spuSaleAttrList" :key="attr.id">
                ...
                <dd class="pointer" v-for="list in attr.spuSaleAttrValueList" :key="list.id" changepirce="0" :class="{ active: list.isChecked=='1' }" @click="changeActive(list, attr.spuSaleAttrValueList)">
                  {{ list.saleAttrValueName }}
                </dd>
              </dl>
            </div>
            <div class="cartWrap">
...
<script>
  ...
  export default {
    ...
    methods: {
      changeActive(origin, originArr) {
        // origin(当前的小对象)和originArr(当前的小对象所属的组) 是一组参数（一个大对象）
      ...
      changeSkuNum(e) {
        let value = e.target.value * 1; // 非法文字NaN
      ...
      async addShopCar() {
        try {
          ...
          // 跳转前将要添加购物车的产品信息 存储于sessionStorage浏览器会话存储
